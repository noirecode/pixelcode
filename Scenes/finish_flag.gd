...
func _on_body_entered(body):
	if (body.name.contains("Character")):
		check_multiple()
		prints("flags_entered: ",get_parent().flags_entered)
...
func check_multiple():
	if get_parent().characters == 1:
		finish_level()
	elif get_parent().flags_entered != (get_parent().characters):
		get_parent().flags_entered += 1
		check_flags()
		
func check_flags():
	if get_parent().flags_entered == get_parent().characters:
		finish_level()


func _on_body_exited(body):
	if (body.name.contains("Character")):
		if get_parent().characters > 1 and get_parent().flags_entered>0:
			get_parent().flags_entered -= 1
